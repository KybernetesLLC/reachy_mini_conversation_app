...
import { mountAccessoryView } from "./views/accessory.js";
...
function readSettingsReturn() {
  try {
    ...
    return [ROUTES.TALK, ROUTES.PERSONALITIES, ROUTES.TOOLS, ROUTES.ACCESSORY].includes(
      route.split("?")[0]
    )
      ? route
      : ROUTES.TALK;
  ...
  function syncHeaderForRoute(route = router.currentRoute() || ROUTES.TALK) {
    ...
    if (back) {
      ...
      if (routeName === ROUTES.SETTINGS) {
        ...
        if (returnName === ROUTES.ACCESSORY) backLabel = "Back to accessory";
      } else if (
        routeName === ROUTES.TOOLS &&
        new URLSearchParams(route.split("?")[1] || "").get("from") === "personalities"
      ) {
